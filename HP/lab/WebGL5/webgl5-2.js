...
/*
** 地球を基準としたときの各惑星の定数
** 大きさ, 自転周期, 公転速度, 公転軌道, 遠日点, 近日点, テクスチャ
*/
function information(size, hertz, velocity, orbital, aphelion, perihelion, texture){
	this.size = size;
	this.hertz = hertz;
	this.velocity = 10 * velocity;
	this.orbital = 20 * orbital;
	this.aphelion = 20 * aphelion;
	this.perihelion = 20* perihelion;
...
Sun = new information(2, null, null, null, null, null, './Sun.jpg');
Earth = new information(1, 1, 1, 1, 1, 1, './Earth.jpg');
Mercury = new information(0.383, 0.016, 1.61, 0.39, 0.47, 0.31, './Mercury.jpg');
Venus = new information(0.950, 0.004, 1.18, 0.72, 0.73, 0.72, './Venus.jpg');
Moon = new information(0.273, 1, 1, 0.1, null, null, './Moon.jpg');
Mars = new information(0.532, 0.97, 0.81, 1.52, 1.67, 1.38, './Mars.jpg');
Jupiter = new information(10.97, 2.4, 0.44, 5.20, 5.46, 4.95, './Jupiter.jpg');
	...
	this.drawLine = function (name){
		...
		for (dw = 0; dw <= 360; dw++){
			var circle = new THREE.Vector3(
				0,
				name.aphelion * name.eccentricity + name.aphelion * Math.cos(dw*Rad),
				name.perihelion * Math.sin(dw*Rad)
			)
...
function render() {	
	...
	this.rotatePlanet = function(mesh, name){
		...
		var name = name;
		mesh.position.x = 0;
		mesh.position.y = name.aphelion * name.eccentricity + name.aphelion * Math.cos(name.velocity * dt * Rad);
		mesh.position.z = name.perihelion * Math.sin(name.velocity * dt * Rad);
	}
	// 衛星の公転
