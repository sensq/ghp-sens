//=====================
// 必要なjsファイルの読み込み
document.write('<script type="text/javascript" src="../three66/build/three.min.js"></script>');
document.write('<script type="text/javascript" src="../three66/examples/js/Detector.js"></script>');
document.write('<script type="text/javascript" src="../three66/examples/js/controls/TrackballControls.js"></script>');

document.write('<script type="text/javascript" src="../three66/examples/js/shaders/CopyShader.js"></script>');
document.write('<script type="text/javascript" src="../three66/examples/js/postprocessing/EffectComposer.js"></script>');
document.write('<script type="text/javascript" src="../three66/examples/js/postprocessing/MaskPass.js"></script>');
document.write('<script type="text/javascript" src="../three66/examples/js/postprocessing/RenderPass.js"></script>');
document.write('<script type="text/javascript" src="../three66/examples/js/postprocessing/ShaderPass.js"></script>');
...
document.write('<script type="text/javascript" src="../util/cannon.js"></script>');
...
function initCamera(posX, posY, posZ, tarX, tarY, tarZ) {
	...
	this.camera2d = new THREE.OrthographicCamera(0, Width, 0, Height, 0.001, 10000);
	// カメラの位置
	this.camera.position = new THREE.Vector3(posX, posY, posZ);
	this.camera2d.position = new THREE.Vector3(posX, posY, posZ);
	// カメラを動かせるようにする（2つ目の引数はマウス入力を受け付けるdiv要素）
	this.controls = new THREE.TrackballControls(this.camera, renderer.domElement);
	this.controls2d = new THREE.TrackballControls(this.camera2d, renderer.domElement);
	// 各種オプション
	...
	this.controls2d.noRotate = true;	// 回転禁止
	this.controls2d.noZoom = true;	// ズーム禁止
	this.controls2d.noPan = true;		// 平行移動禁止

	// 向いている方向
	this.controls.target = new THREE.Vector3(tarX, tarY, tarZ)
	this.controls2d.target = new THREE.Vector3(tarX, tarY, tarZ)
};
...
function BlenderAnimationPrepare(startframe, keyframes, duration){
	var anim = {	//アニメーション用
		startframe: startframe, // 開始フレーム
		keyframes: keyframes,   // 開始から終了までのフレーム数
		duration: duration, // アニメーションの時間[ms]
		interpolation: 0, // 1フレームあたりの時間（FPSの逆数）
		lastKeyframe: false,    // 前フレーム
		currentKeyframe: true,	// 現在フレーム
		move: false,	// インタラクション用
	};
...
/*
** Blenderアニメーションループ
** loadBlenderとBlenderAnimationPrepareを先に実行しておくこと
*/
function BlenderAnimation(anim){
	...
	if (keyframe != anim.currentKeyframe){
		meshBlender.morphTargetInfluences[anim.lastKeyframe] = false; // 前フレーム
		meshBlender.morphTargetInfluences[anim.currentKeyframe] = true; // 現在フレーム
		meshBlender.morphTargetInfluences[keyframe] = false; // 次フレーム
		anim.lastKeyframe = anim.currentKeyframe; // 前フレームを1フレーム更新
		anim.currentKeyframe = keyframe; // 現在フレームを1フレーム更新
	}
